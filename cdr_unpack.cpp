...
int unpack::get_bin(unsigned char *a, int length)
{
	int bin_value = 0;

	for (int n=0; n<length; n++)
	{
		bin_value = *a + (bin_value * 256);
		a++;
	}
	return bin_value;
}
...
// copy ascii characters around - trims space charactors
...
char* unpack::get_hex(unsigned char *a, int length, char *buf)
{
	int k;
	unsigned char b,c;
	for(k=0; k<length*2; k++)
...
char* unpack::get_THEX(unsigned char *a, int length, char *buf)
{
	int k;
	unsigned char b,c;
	for(k=0; k<length*2; k++)
